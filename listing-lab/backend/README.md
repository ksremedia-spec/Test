...
| `GET /api/photos/<key>?preview=1` | A small copy for the app's My photos grid (10 Sep 2026): 640 px wide, made once by Cloudflare's image tool (the `IMAGES` binding, free for 5,000 unique shrinks a month) the first time it is asked for, kept in R2 as `<key>-preview.jpg`. Old photos get theirs on first request; nothing to migrate. If anything stops a preview being made, the full photo is served. `GET /api/jobs` now carries `previewUrl`. | one shrink per photo, ever |
| `POST /api/devices`, `DELETE /api/devices/:token` | Push notifications (10 Sep 2026): the app registers its phone; when a job finishes — delivered, rejected or failed — every phone on the account gets one line (`Your Twilight is ready.` / `Your Twilight came back — credits returned.`) through Apple's push service, signed with the `.p8` key in `APNS_PRIVATE_KEY` (`src/apns.js`). With no key configured nothing is sent and nothing breaks. Dead tokens are forgotten. New `devices` table, migration 012. | one push per phone per finish; a late duplicate callback sends nothing |
...
5. **Push notifications** (10 Sep 2026): in the Apple Developer portal, Certificates, Identifiers & Profiles → Keys → **+** → name it, tick **Apple Push Notifications service (APNs)**, Continue, Register, **Download** the `.p8` file (Apple lets you download it once — keep it). Note the **Key ID** on that page. Then from `backend/`:
   `npx wrangler secret put APNS_KEY_ID` (paste the Key ID), `npx wrangler secret put APNS_TEAM_ID` (paste `6YCK9MG5RD`), `npx wrangler secret put APNS_PRIVATE_KEY` (paste the whole `.p8` file's text, BEGIN to END lines included). No redeploy needed — secrets take effect at once. Also enable **Push Notifications** on the App ID `com.horizonhomemedia.listinglab` (Xcode does this itself on the next build with automatic signing).
...
7. **FAQ wording** (approved 10 Sep 2026): the answer to "Are my photos private?" in `web/faq.html` now ends with *"You can also delete your account — and every photo with it — from the iPhone app, under Account."* It ships with the deploy.
