...
5. **Push notifications** (10 Sep 2026): the Apple push key is per developer team, so Listing Lab reuses the Horizon Home Media app's (`AuthKey_7GF2569W58.p8`, on the Mac Desktop). `APNS_KEY_ID` and `APNS_TEAM_ID` are identifiers and live in `wrangler.toml`; the key itself is the secret: from `backend/`, `npx wrangler secret put APNS_PRIVATE_KEY < ~/Desktop/AuthKey_7GF2569W58.p8`. No redeploy needed. The App ID `com.horizonhomemedia.listinglab` needs the **Push Notifications** capability (Xcode adds it on the next build with automatic signing).
